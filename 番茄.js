...
function updateTomatoGrowth() {
    ...
    if (currentStage) {
        // 計算容器的大小
        const containerWidth = document.getElementById('tomato-container').offsetWidth;
        const containerHeight = document.getElementById('tomato-container').offsetHeight;

        // 計算最大縮放比例，避免圖片超出容器範圍
        const maxScale = Math.min(containerWidth / 100, containerHeight / 150);

        // 根據當前的 scale 和最大縮放比例計算縮放比例
        ...
        if (currentStage.image !== lastImage) {
            ...
            }, 500); // 等待淡出完成，這裡的時間應大於 CSS 中的 `opacity` 過渡時間
        }
    }
